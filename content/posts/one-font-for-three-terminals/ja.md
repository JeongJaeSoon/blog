...
差はそのフェイスに入れたストローク幅と同じです。Mediumは1段（27）、Blackは3段（81）をまるごともう一度食っていたことになります。該当するグリフは1,412字、エンボールドが入る10フェイスすべてに出ていました。`Agrave`、`Aacute`、`Adieresis`のようなアクセント付きラテンと、`onehalf`のような分数記号です。ハングルの音節は単純なコンターなので無事でした。
