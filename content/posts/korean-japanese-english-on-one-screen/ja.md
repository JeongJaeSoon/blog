...
summary: >-
  ...
  3つとも持っている等幅フォントはなく、VSCode・iTerm2・Orcaはそれぞれ別のフォントで穴を
  埋めていました。まとめる作業は早く終わり、時間を取られたのはアプリ側のレンダリング設定でした。
lang: ja
tags:
  - fonts
  - terminal
...
この3つを全部持っている等幅フォントがありません。無い文字はアプリがフォールバックで埋めるのですが、VSCodeとiTerm2とOrcaはそれぞれ別のフォントを選びました。3つを並べて同じファイルを開くと、ハングルが3つとも違って見えました。サイズはどれも14pxに揃えてあります。
