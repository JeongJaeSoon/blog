---
title: 韓国語・日本語・英語が同じ画面に混ざる開発環境でフォントを決める
date: '2026-09-22'
summary: >-
  コミットメッセージは韓国語、コメントとログは日本語、コードは英語。それが1つの画面に混ざります。
  VSCode・iTerm2・Orcaはそれぞれ別のフォントで穴を埋めていて、同じ文字がアプリごとに違って
  見えました。まとめる作業は早く終わり、時間を取られたのはアプリ側のレンダリング設定でした。
lang: ja
tags:
  - fonts
  - terminal
...
日本で開発の仕事をしていると、韓国語と日本語と英語をIDEやターミナルに出すことになります。ところが、3つの見た目が揃う等幅フォントになかなか出会えませんでした。たとえばこんな行です。
...
どのフォントもこの3つを揃えて持っていません。無い文字はアプリがフォールバックで埋めるのですが、VSCodeとiTerm2とOrcaはそれぞれ別のフォントを選びました。3つを並べて同じファイルを開くと、ハングルが3つとも違って見えました。サイズはどれも14pxに揃えてあります。

フォントを1つにまとめれば片付く話だと思っていました。まとめる作業自体はスクリプト3本で終わり、時間がかかったのはその先です。同じフォントを同じサイズで指定しても3つの見た目は揃いませんでした。原因はほとんどフォントファイルの外にあり、中にあった1つもiTerm2だけが読むメタデータでした。

## 1行が3つのフォントから出てくる

さきほどの行を文字ごとに分けると、出典はこう割れます。
...
フォントが3つなら太さも3つ、線の強弱も3つです。同じ行でハングルだけ細かったり漢字だけ太かったりすると、1文字ずつ見ておかしいわけではありません。行全体が不揃いに見えます。1行に3つの言語が来るのは例外ではなく毎日なので、このずれが1日じゅう目に残りました。

既製品ならSarasa Term Kなどの選択肢があります。それでも自分で合成したのは、使い慣れたMenloのラテン文字と行間をそのまま残したかったからです。

## フォールバックの指定を3つに共通化できない

まず詰まったのは、フォールバックをアプリ設定で片付けようとした側でした。3つのアプリはフォントフォールバックの扱いがそれぞれ違います。
...
`prep.py`が3つのソースを2048 upemに揃え、`GSUB`・`GPOS`・`GDEF`・`DSIG`・`morx`・`kern`を落とします。ここで捨てているのはシェーピングで、ターミナル用途では困りません。ただし合字を有効にして使うなら`GSUB`は残す必要があります。D2Codingはupemが1000なので、`scaleUpem`で2048まで上げます。

`merge.py`がその3つを1つに結合します。cmapが衝突したときは先頭のフォントが勝つので、さきほどの出典表の並び順がそのまま優先順位です。
...
合成後のグリフは54,587字です。ビルドは手元のMacで6.4分かかります。

## 14pxで見分けられる太さの差は27が下限だった
...
代償が1つあります。700はもうMenloが描いたBoldではなく、それより1段太い合成です。ターミナルのANSIボールドは700を使うので、ボールドが原本より太くなります。OrcaとVSCodeならボールドウェイトを600に下げて描かれたBoldを取り戻せます（`Bold Font Weight`、`terminal.integrated.fontWeightBold`）。iTerm2にはその設定がありません。ボールドのフェイスを`fsSelection`のboldビットで選んでいて、そのビットは700にしか立っていないからです。3つを揃えるのが目的なら、既定は700で、600は残り2つでしか使えない選択肢になります。

## アクセント付きラテン1,412字が二重に太っていた

`embolden()`がグリフ順に回りながら`glyf`をその場で置き換えていました。合成グリフは描画時にコンポーネントを`glyf`から実時間で取り出すので、コンポーネントが順序として先に来ていると、すでに太くなったコンポーネントにもう一度ストロークが乗ります。`A`と`Aacute`のグリフ幅を並べて測ると、そのまま出ます。

| フェイス | A | Aacute | 差 |
|---|---:|---:|---:|
| Regular（原本） | 1159.0 | 1159.0 | 0.0 |
| Medium | 1185.8 | 1212.6 | +26.8 |
| Bold | 1193.5 | 1220.7 | +27.2 |
| Black | 1247.3 | 1327.7 | +80.4 |

差はそのフェイスに入れたストローク幅と同じです。Mediumは1段（27）、Blackは3段（81）をまるごともう一度食っていたことになります。該当するグリフは1,412字、実際に`embolden()`を通る8フェイス（Medium・Bold・ExtraBold・Blackとそのイタリック）すべてに出ていました。`Agrave`、`Aacute`、`Adieresis`のようなアクセント付きラテンと、`onehalf`のような分数記号です。ハングルの音節は単純なコンターなので無事でした。`steps`が0のRegular・SemiBoldとそのイタリックは`embolden()`に入りません。表のRegularが0.0になっているのは、そこを通っていないからです。

変形に入る前にアウトラインを全部スナップショットするよう直しました。ビルドし直して測ると、ローマン体6種の差はすべて0.0です。イタリックに残る42と88は、傾いたアクセントがもともとバウンディングボックスを広げている分で、同じベースの中で一定なので（400・500が42、600〜900が88）二重ストロークではありません。

見つけたのは自分ではありません。14pxのターミナルで`Aacute`に出くわすことはなく、出くわしても気づけた気がしません。画面ではなくコードを読んだCodexのレビューが拾いました（[JeongJaeSoon/menlocjk#1](https://github.com/JeongJaeSoon/menlocjk/pull/1)）。
...
冒頭の3行が、3つのアプリで同じ太さで出るようになりました。ハングルも漢字もラテン文字も線の太さが揃うので、行が不揃いに見えません。アプリを移っても文字は変わらないままです。
...
Menloが混ざっている限り、成果物は自分のMacの中だけで使います。[JeongJaeSoon/menlocjk](https://github.com/JeongJaeSoon/menlocjk)にスクリプトだけあって`.ttf`がないのはそのためです。導入手順もそちらに書いたので、各自が手元でビルドします。
