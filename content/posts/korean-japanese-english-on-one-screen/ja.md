...
date: '2026-09-22'
summary: >-
  日本で働いていると、ラテン文字とかな・漢字とハングルが同じ行に並びます。3つとも持っている
  等幅フォントはなく、VSCode・iTerm2・Orcaはそれぞれ別のフォントで穴を埋めていました。
  1つのファミリーにまとめる作業は早く終わり、時間を取られたのはアプリ側のレンダリング設定でした。
lang: ja
tags:
  - fonts
  - terminal
...
日本で働いているので、1日に読む文字が3種類あります。考えるのは韓国語、チケットもコメントもログも日本語、コードは英語です。3つがファイルごとに分かれてくれるわけでもありません。画面に出てくるのはだいたいこういう形です。
...
3行の中にラテン文字とかな・漢字とハングルが全部入っています。3つとも持っている等幅フォントはありません。無い文字はアプリがフォールバックで埋めるのですが、VSCodeとiTerm2とOrcaはそれぞれ別のフォントを選びました。3つを並べて同じファイルを開くと、ハングルが3つとも違って見えました。サイズはどれも14pxに揃えてあります。

フォントを1つにまとめれば片付く話だと思っていました。まとめる作業自体はスクリプト3本で終わりました。問題はその先です。同じフォントを同じサイズで指定しても3つの見た目は揃いませんでした。原因はほとんどフォントファイルの外にあり、中にあった1つもiTerm2だけが読むメタデータでした。
