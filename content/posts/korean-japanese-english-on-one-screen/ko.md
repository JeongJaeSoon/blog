...
date: '2026-09-22'
summary: >-
  커밋 메시지는 한국어, 주석과 로그는 일본어, 코드는 영어로 한 화면에 섞입니다. 셋을 다
  가진 고정폭 폰트가 없어 VSCode·iTerm2·Orca가 각자 다른 폰트로 메꿨습니다. 셋을 한
  패밀리로 합치고 나서 시간을 쓴 쪽은 폰트가 아니라 앱의 렌더링 설정이었습니다.
lang: ko
tags:
  - fonts
  - terminal
...
이 셋을 다 가진 고정폭 폰트가 없습니다. 없는 글자는 앱이 폴백으로 메꾸는데, VSCode와 iTerm2와 Orca가 각자 다른 폰트를 골랐습니다. 그래서 셋을 나란히 띄워 놓고 같은 파일을 열면 한글이 셋 다 다르게 보였습니다. 크기는 전부 14px로 맞춰 놨는데도 그랬습니다.
