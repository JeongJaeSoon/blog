---
title: 한국어·일본어·영어가 한 화면에 섞이는 개발 환경에서 폰트 정하기
date: '2026-09-22'
summary: >-
  커밋 메시지는 한국어, 주석과 로그는 일본어, 코드는 영어로 한 화면에 섞입니다. VSCode와
  iTerm2와 Orca가 각자 다른 폰트로 구멍을 메꿔서 같은 글자가 앱마다 달라 보였습니다. 셋을
  한 패밀리로 합치고 나서 시간을 쓴 쪽은 폰트가 아니라 앱의 렌더링 설정이었습니다.
lang: ko
tags:
  - fonts
  - terminal
...
일본에서 개발자로 일하다 보면 한국어와 일본어, 영어를 IDE 혹은 터미널에 출력하게 됩니다. 그런데 셋을 동시에 예쁘게 그려 주는 고정폭 폰트가 없어서 오래 고민했습니다. 예를 들면 이런 줄입니다.
...
어느 폰트도 이 셋을 다 갖고 있지 않습니다. 없는 글자는 앱이 폴백으로 메꾸는데, VSCode와 iTerm2와 Orca가 각자 다른 폰트를 골랐습니다. 그래서 셋을 나란히 띄워 놓고 같은 파일을 열면 한글이 셋 다 다르게 보였습니다. 크기는 전부 14px로 맞춰 놨는데도 그랬습니다.

폰트 하나로 합치면 끝날 문제라고 봤습니다. 합치는 쪽은 실제로 스크립트 세 개로 끝났고, 시간은 그다음에 갔습니다. 세 앱에 같은 폰트를 물리고 같은 크기를 넣었는데도 여전히 다르게 보였습니다. 원인은 대부분 폰트 파일 바깥에 있었고 안에 있던 하나는 iTerm2만 읽는 메타데이터였습니다.

## 한 줄이 세 폰트에서 나옵니다

위 예시를 글자별로 나누면 출처가 이렇게 갈립니다.
...
폰트가 셋이면 굵기도 셋이고 획 대비도 셋입니다. 같은 줄 안에서 한글만 가늘거나 한자만 두꺼우면, 한 글자씩 뜯어봐서 이상한 데를 찾기는 어렵습니다. 줄 전체가 고르지 않게 보입니다. 한 줄 안에 세 언어가 같이 오는 건 예외가 아닙니다. 매일이라 이 어긋남이 하루 종일 눈에 남았습니다.

기성품으로는 Sarasa Term K 같은 선택지가 이미 있습니다. 직접 합친 이유는 쓰던 Menlo의 라틴 글자와 줄 간격을 그대로 두고 싶어서입니다.

## 폴백 체인을 세 앱에 똑같이 넣는 방법이 없습니다

폴백을 앱 설정으로 해결하려던 쪽부터 막혔습니다. 세 앱이 폰트 폴백을 다루는 방식이 전부 다릅니다.
...
`prep.py`가 세 소스를 2048 upem으로 맞추고 `GSUB`, `GPOS`, `GDEF`, `DSIG`, `morx`, `kern`을 떼어 냅니다. 터미널에서 쓸 폰트라 셰이핑 기능을 버린 것인데, 합자를 켜고 쓸 생각이라면 `GSUB`는 남겨야 합니다. D2Coding은 upem이 1000이라 `scaleUpem`으로 2048까지 올립니다.

`merge.py`가 그 셋을 하나로 붙입니다. cmap이 겹치면 앞 폰트가 이기므로 위 출처 표의 순서가 곧 우선순위입니다.
...
## 14px에서 눈에 보이는 굵기 차이는 27이 최소였습니다
...
대가가 하나 있습니다. 700은 더 이상 Menlo가 그린 Bold가 아닙니다. 그보다 한 칸 굵은 합성입니다. 터미널의 ANSI 볼드는 700을 쓰니 볼드가 원본보다 굵어집니다. Orca와 VSCode에서는 볼드 웨이트를 600으로 내려 원본 Bold를 되찾을 수 있습니다(`Bold Font Weight`, `terminal.integrated.fontWeightBold`). iTerm2에는 그 설정이 없습니다. 볼드 페이스를 `fsSelection`의 bold 비트로 고르는데 그 비트는 700에만 있기 때문입니다. 세 앱을 같게 맞추는 게 목적이라면 700이 기본이고 600은 나머지 둘에서만 쓸 수 있는 선택지입니다.

## 악센트 라틴 1,412자가 두 번 굵어져 있었습니다
...
차이가 그 페이스에 들어간 스트로크 폭과 같습니다. Medium은 한 칸(27), Black은 세 칸(81)을 통째로 한 번 더 먹었다는 뜻입니다. 해당하는 글리프는 1,412자입니다. 엠볼딩이 실제로 도는 8개 페이스(Medium·Bold·ExtraBold·Black과 그 이탤릭) 전부에 있었습니다. `Agrave`, `Aacute`, `Adieresis` 같은 악센트 라틴과 `onehalf` 같은 분수 기호입니다. 한글 음절은 단순 컨투어라 무사했습니다. `steps`가 0인 Regular·SemiBold와 그 이탤릭은 `embolden()`을 아예 타지 않습니다. 위 표에서 Regular만 0.0인 이유입니다.
...
맨 위의 그 세 줄이 이제 세 앱에서 같은 굵기로 나옵니다. 한글과 한자와 라틴이 같은 획 두께를 쓰니 줄이 고르게 보이고 앱을 옮겨 다녀도 글자가 바뀌지 않습니다.
...
Menlo가 섞여 있는 한 결과물은 제 맥 안에서만 씁니다. [JeongJaeSoon/menlocjk](https://github.com/JeongJaeSoon/menlocjk)에 스크립트만 있고 `.ttf`가 없는 이유입니다. 설치 절차도 거기 적어 뒀고, 각자 자기 맥에서 빌드해야 합니다.
...
Orca의 500은 세 앱을 나란히 놓고 눈으로 맞춘 값입니다. 계산해서 나온 값이 아닙니다. Orca가 `-webkit-font-smoothing`을 언젠가 걷어내면 이 보정은 틀린 값이 됩니다. 플러그인에서 CSS를 건드릴 방법이 생기면 폰트 쪽 보정을 지우고 스무딩을 직접 끄는 편이 맞습니다.
