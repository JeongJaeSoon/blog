...
date: '2026-09-22'
summary: >-
  일본에서 일하면 한 줄 안에 라틴 글자와 가나·한자와 한글이 같이 들어옵니다. 어느 폰트도
  셋을 다 갖고 있지 않아서 VSCode·iTerm2·Orca가 각자 다른 폰트로 메꿨습니다. 셋을 한
...
일본에서 일합니다. 그래서 하루에 읽는 글자가 세 종류입니다. 생각은 한국어로 하고, 티켓과 주석과 로그는 일본어로 읽고, 코드는 영어로 씁니다. 셋이 파일을 나눠 갖지도 않습니다. 화면에 뜨는 모양은 대충 이렇습니다.
...
라틴 글자와 가나·한자와 한글이 세 줄 안에 다 있습니다. 그리고 이 셋을 다 갖고 있는 고정폭 폰트가 없습니다. 없는 글자는 앱이 폴백으로 메꾸는데, VSCode와 iTerm2와 Orca가 각자 다른 폰트를 골랐습니다. 셋을 나란히 띄워 놓고 같은 파일을 열면 한글이 셋 다 다르게 보였습니다. 크기는 전부 14px로 맞춰 놨는데도 그랬습니다.

폰트 하나로 합치면 끝날 문제라고 봤습니다. 합치는 쪽은 실제로 스크립트 세 개로 끝났습니다. 문제는 그다음이었습니다. 세 앱에 같은 폰트를 물리고 같은 크기를 넣었는데도 여전히 다르게 보였습니다. 원인은 대부분 폰트 파일 바깥에 있었고 안에 있던 하나는 iTerm2만 읽는 메타데이터였습니다.
